...
# print(ips)
# print(ipv6s)
br_defaults = ["br_default", "domain_default", "macOs_is_bad", "test_default"]
...
# print(VLANS[0])
cont_addr1 = randrange(0,len(ips))
cont_addr2 = randrange(0,len(ips))
const_br = randrange(0,len(br_defaults))
...
'https://10.170.6.4:8765/nvue_v1/vrf/default/router/ospf/area/%s?rev='% areas[0],
'https://10.170.6.4:8765/nvue_v1/vrf/default/router/ospf/area/%s?rev='% areas[0],
'https://10.170.6.4:8765/nvue_v1/bridge/domain/%s/vlan/%s?rev='% (br_defaults[const_br],VLANS[0]),
'https://10.170.6.4:8765/nvue_v1/bridge/domain/%s/vlan/%s?rev='% (br_defaults[const_br],VLANS[1]),
'https://10.170.6.4:8765/nvue_v1/bridge/domain/%s/vlan/%s?rev='% (br_defaults[const_br],VLANS[2]),
...
D = ["'{\"address\": {\"6.6.6.6/32\": {}}}'", "'{\"address\": {\"%s\": {}}}'"% ips[randrange(0,len(ips))],"'{\"address\": {\"%s\": {}}}'"% ipv6s[randrange(0,len(ipv6s))],"'{\"address\": {\"%s\": {}}}'"% ips[randrange(0,len(ips))],"'{\"address\": {\"%s\": {}}}'"% ipv6s[randrange(0,len(ipv6s))],"'{\"address\": {\"%s\": {}}}'"% ips[randrange(0,len(ips))], "'{\"address\": {\"%s\": {}}}'"% ipv6s[randrange(0,len(ipv6s))], "'{\"address\": {\"%s\": {}}}'"% ips[randrange(0,len(ips))], "'{\"address\": {\"%s\": {}}}'"% ipv6s[randrange(0,len(ipv6s))], "'{\"%s\": {}}'"% bonds[0], "'{\"member\": {\"%s\": {}}}'"% swps[randrange(1,len(swps))],"'{\"mlag\": {\"id\": %s}}'"% random.randrange(1,10),"'{\"br_default\": {\"access\": %s}}'"% VLANS[0],"'{\"untagged\": 1}'","'{\"vlan\" : {\"%s\": {}}}'"% VLANS[0],
     "'{\"address\": {\"%s\": {}}}'"% ips[randrange(0,len(ips))], "'{\"member\": { \"%s\": {}, \"%s\": {}}}'"% (swps[2], swps[3]), "'{\"mac-address\": \"%s\"}'"% mac,"'{\"backup\": {\"5.5.5.5\" : {\"vrf\": \"default\"}}}'","'{\"%s\": {\"access\": %s}}'"% (br_defaults[const_br],VLANS[1]),"'{\"vlan\" : {\"%s\": {}}}'"% VLANS[1],"'{\"address\": {\"%s\": {}}}'"% ips[randrange(0,len(ips))],"'{\"router-id\": \"6.6.6.6\"}'",
     "'{\"area\": {\"0\": {}}}'", "'{\"network\": {\"6.6.6.6/32\": {}}}'","'{\"network\": {\"%s\": {}}}'"% ips[cont_addr1],"'{\"network\": {\"%s\": {}}}'"% ips[cont_addr2],"'{\"vni\": {\"%s\": {}}}'"% VLANS[0],"'{\"vni\": {\"%s\": {}}}'"% VLANS[1],"'{\"vni\": {\"%s\": {}}}'"% VLANS[2],"'{\"source\": {\"address\": \"5.5.5.5\"}}'",
